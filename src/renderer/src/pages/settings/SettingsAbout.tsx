...
const UPDATE_HISTORY: HistoryEntry[] = [
  {
    version: 'v1.3.2',
    date: '2026-09-21',
    notes: '统计页说实话的一版：图上画的、标题写的、徽标挂的，终于是同一个数。',
    details: [
      '• 趋势图不再骗人：选「近 30 天」以前只画最后 7 根柱子，选「本年」会把按天的数据标成月份；现在时间名字和柱子粒度由同一份口径决定',
      '• 复习热力补上今天的格子：以前日期按 UTC 切，凌晨打开应用会把「今天」算成昨天，整列错位',
      '• 「卡片总数」不再冒充「复习卡片」，热力图的角标改成这 12 周真实复习次数',
      '• 调用记录里的「费用」列删掉了 —— 数据库那一列从来没有写入方，永远显示 ¥0.00，容易让人以为真没花钱',
    ],
  },
  {
    version: 'v1.3.1',
    date: '2026-09-21',
    notes: '修一处「查了但没人知道」：应用启动确实会查更新，以前查到的结果没人接，界面就永远显示没有新版。',
    details: [
  ...
  },
  {
    version: 'v1.3.0',
    date: '2026-09-21',
    notes: '会记摘要、说真话版：书籍层级摘要上线，界面上的假数字与断线字段全面清扫。',
    details: [
      ...
      '• 界面数字对账：档案页、复习计数、复习导出 CSV、卡片复习次数逐条追到真实数据库列，追不到的直接删掉',
      '• 离线可用：中文字体随包分发，不再从 Google Fonts 拉取',
      '• 品牌：新徽标「玉璧」+ 转曲字标，全部色值收进单一真值文件',
      '• 精简：砍掉 26 条界面早已不用的后端链路，7 个巨型页面拆分',
    ],
  },
  {
    version: 'v1.2.0',
  ...
  },
  {
    version: 'v1.1.0',
    date: '2026-08-28',
    notes: '维护迭代版：复习闭环落地、画像注入 AI、界面去伪存真。',
    details: [
      ...
      '• 界面治理：删除假数据与死链，智能体编排迁入设置，书籍详情展示真实动态',
