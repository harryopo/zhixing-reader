...
import { TokenSummary } from 'src/types/renderer'
...
export default function Sidebar() {
  ...
  const loadTokenData = async () => {
    ...
    try {
      ...
      setTokenSummary(stats)
    } catch (error) {
      console.error('加载token数据失败:', error)
    }
  }

  const inputRatio = tokenSummary && tokenSummary.totalTokens > 0
    ...
    <aside className="w-64 bg-white border-r border-border flex flex-col h-full">
      <div className="p-6 border-b border-border-light">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-text-primary font-serif">知行读书</h1>
            <p className="text-xs text-text-tertiary">AI 阅读成长助手</p>
          </div>
        </div>
      </div>
        ...
        <div className="p-3 border-t border-border-light">
          <div className="rounded-lg bg-surface-secondary p-3">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-xs text-text-tertiary">Token 消耗</span>
              <span className="text-xs font-semibold text-primary">{formatTokens(tokenSummary.totalTokens)}</span>
            </div>
            <div className="relative h-1.5 rounded-full bg-gray-200 overflow-hidden">
              <div
                ...
                style={{ width: `${Math.max(inputRatio, 0)}%` }}
              />
            </div>
            <div className="flex items-center justify-between mt-1.5">
              <span className="text-[10px] text-text-tertiary">{tokenSummary.totalRequests} 次请求</span>
      ...
      <div className="px-6 pb-4 pt-2">
        <p className="text-xs text-text-tertiary text-center">知行读书 v1.0.0</p>
      </div>
    </aside>
  )
}