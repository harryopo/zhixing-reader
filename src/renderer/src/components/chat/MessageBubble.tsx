  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 操作栏 */}
        {!isStreaming && content && (onCopy || onRegenerate || onToggleLike || onToggleBookmark) && (
          <div
            className="msg-actions"
            style={{ display: 'flex', gap: 'calc(var(--spacing) * 2)' }}
          >
            {onToggleLike && (
              <ActionButton label="点赞" onClick={() => onToggleLike(!liked)} active={liked}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill={liked ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
            ...
            {onToggleBookmark && (
              <ActionButton label="收藏" onClick={() => onToggleBookmark(!bookmarked)} active={bookmarked}>
...
function ActionButton({
  ...
  const restColor = active ? 'var(--primary)' : 'var(--muted-foreground)'
  const restBorder = active ? 'var(--primary)' : 'var(--border)'
  return (
    <button
      type="button"
      aria-label={label}
