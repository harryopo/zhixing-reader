...
| `npm run test` | vitest 全通过（860 用例）| 手动 + CI |
...
*最后更新：2026-09-16 | 数据血缘修复（5 个断点 + 启动自动修复）+ 门禁基线刷新（860 用例 / 39 文件）*
