...
### Fixed
- **微信读书后台自动同步不再抹掉阅读进度**：按频率在后台跑的那条同步通路，每次都会把库里每本书的阅读进度写成 0 —— 它取的那个接口不返回进度，代码里却按"返回了"去写。现在手动同步与后台自动同步共用同一份写库计划，更新时不再带进度列（进度仍由按本查询单独缓存）
- **后台自动同步不再按书名判重**：同名但不同版本的两本书，第二本一直进不来，还会把第一本的作者与封面覆盖掉；现在两条通路都按书的唯一 id 判
- **自动同步失败后不再连续重试**：密钥失效或网络不通时，"下一次什么时候跑"被算成当前这一刻，于是请求连发、界面反复弹「自动同步失败」。现在失败后按一小时退避，与每小时兜底检查同一节律；关掉了开关就不再续排
- **自动同步一本书都没写进库时不再报告成功**：过去全部写入失败仍按成功广播，界面上看不出问题；现在如实报失败并保留重试
- **「测试连接」现在会说清真的拉到了哪一本书**：微信读书的连接测试原本只回一句"连接成功"，界面上看不出它究竟取到了什么。现在成功提示会带上书架上的第一本书（「连接成功！已拉到《书名》」），一本都没有时如实只说连接成功
- **API Key 失效时不再反复重试**：认证类错误（401 / 403）原本靠"错误文案里有没有 401 字样"来判定要不要重试，遇到服务端返回中文说明时会判定落空，一个失效的 key 会被白白重试三次。现在按错误类型判定，一次就如实报失败
...
### Changed
- **覆盖率统计范围如实扩大**：一批"已有测试、却没被统计到"的主进程与界面模块（对话编排、设置与提示词服务、启动修复、章节摘要、弹层组件等 10 个文件）进入了门禁统计，聚合覆盖率 89.95% → 90.88%。同时新增一条检查：以后再有"有测试却没统计、也没登记"的文件，门禁直接判红；剩下 22 个统计不达标的文件按实测数字逐条登记为待办（不再是"看情况决定要不要测"）
- **移除主进程内重复的一套数据访问层**：`electron/repositories/` 与 `electron/database/` 读写的是同一批表，两套实现长期各自演进 —— 复习统计的"待复习卡片数"两边口径就不一样（一套排除了从未学过的卡片，另一套没排除）。这一层只有 5 个方法还在被走到（检索与用户画像取数），已改到与界面同一条读法上；其余连同类型定义一并撤下，撤掉 1559 行、全批净减 1347 行。新增一条检查钉住：这套第二数据访问层不许再回来；用户画像服务另加了一组"对真实数据库对账"的检查（此前只有把数据库整体假掉的那类测试，证不了真实那一行读出来是什么形状）
