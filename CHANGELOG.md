...
### Added
- **删除可以在 8 秒内撤销**：划线、知识卡片、方法论、生词这四类在删除前会先在主进程里留一份现场（删一条划线会连它的复习卡片与复习记录一起留着），提示条上给一个「撤销」，点一下原样恢复，复习进度不受影响。现场只存在内存里、最多 20 次，应用重启即失效，所以出口就是删除后那一条提示。同时移除 5 条界面上已无消费者的直接删除通道（161 条通道）
