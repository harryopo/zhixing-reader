...
const SPECS: KindSpec[] = [
  ...
  {
    kind: 'card',
    columns: `k.id, k.book_id, k.title, k.content, k.interpretation, k.application, k.type, b.title AS book_title`,
    from: `FROM knowledge_cards k JOIN books b ON k.book_id = b.id`,
    // 与知识卡片页那台筛选器搜同样的字段（标题 / 正文 / 解读 / 应用 / 书名）：
    // 少一列，「共 N 条」点过去就会少筛出几条（tests/search-filter-parity.test.ts 逐条对账）
    likeColumns: ['k.title', 'k.content', 'k.interpretation', 'k.application', 'b.title'],
    order: 'k.created_at DESC',
    textColumns: ['content', 'interpretation', 'application', 'title', 'book_title'],
    title: (r) => str(r.title),
    meta: (r) => str(r.book_title),
  },
  {
    kind: 'methodology',
    columns: `m.id, m.book_id, m.name, m.name_en, m.description, m.steps, m.trigger_scenario,
              m.output_format, m.examples, b.title AS book_title`,
    from: `FROM methodologies m JOIN books b ON m.book_id = b.id`,
    // 同方法论页那台筛选器：名字 / 英文名 / 触发场景 / 说明 / 输出格式 / 示例 / 步骤 / 书名
    likeColumns: [
      'm.name',
      'm.name_en',
      'm.trigger_scenario',
    ...
    textColumns: ['description', 'steps', 'trigger_scenario', 'examples', 'name', 'book_title'],
    title: (r) => str(r.name),
    meta: (r) => str(r.book_title),
  },
  {
