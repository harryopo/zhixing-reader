...
- examples: 示例（字符串）
- tags: 标签数组（字符串数组）
- sourceIndexes: 这条方法论主要依据的笔记编号数组（数字数组）。上面每条笔记前都有 [1] [2] 这样的编号，请填对应数字。若是综合多条，按重要性最多填 3 个。
...
- tags: 标签数组（字符串数组）
- sourceIndex: 这张卡主要依据的笔记编号（数字）。上面每条笔记前面都有 [1] [2] 这样的编号，请填对应的那个数字。如果一张卡综合了多条笔记，填最主要的那一条。
