...
[![Tests](https://img.shields.io/badge/tests-1226%20%E7%94%A8%E4%BE%8B%20/%2079%20%E6%96%87%E4%BB%B6-22c55e)](./tests)
...
| **代码规模** | 56,371 行 TypeScript strict（`electron/` + `src/` 下跟踪的 `.ts`/`.tsx`，2026-09-25 实测）|
| **测试** | 1226 用例 / 79 文件（`npm run test`，**不含覆盖率**）· 覆盖率门禁是另一条命令 `npm run test:cov`，阈值 lines 83 / branches 80 / functions 75 / statements 83，**只作用于 `vitest.config.ts` 的 include 清单（实测 51 个文件），不是全库覆盖率**；CI 从 2026-09-24 起跑的就是这条命令，阈值不过则流水线红 |
...
| **测试** | Vitest | 3.x | 1226 用例 / 79 文件，阈值见 `vitest.config.ts` |
...
**五层架构**：Renderer（React SPA）→ Preload（contextBridge 安全桥，527 行）→ IPC（`electron/ipc/` 11 个领域 handler + `index.ts` 统一注册 + `types.ts` 契约，共 **166 条通道**）→ Service/Agent（RAG / FSRS / 智能体编排）→ Data（sql.js 16 张表 + safeStorage）。
...
│   ├── database/                            # sql.js DB（按领域 15 文件 + index / schema / connection）
...
├── tests/                                   # Vitest 单元测试（1226 用例 / 79 文件）
├── .github/
...
# 覆盖率是另一条命令：只对 vitest.config.ts 的 include 清单（实测 51 个文件）统计
...
*最后更新：2026-09-25 | 与 master 分支代码一致（最新发布 v1.3.4，1226 用例 / 79 文件）*
