...
> **最近核验**：2026-09-23（运行时批次：Electron 35.7.5 → **39.8.10**，Issue #11 的 32 条告警按 GitHub 依赖图逐条重算全清，重新出包并跑起主进程；顺带查出并修掉「开发版 settings/日志其实一直写在装机版目录」这条时序缺陷；同日更早：semver 范围内刷新锁文件，85 条 Dependabot 告警清掉 33 条、剩 52 条待跨大版本；GitHub 维护一轮 —— CI 长期红的根因是检出换行符、补 issue/PR 模板与 SECURITY.md、README 数字按代码逐条重核、建 10 条 backlog issue；再早：v1.3.4 出包并发布）
