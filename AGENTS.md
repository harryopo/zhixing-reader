...
> **最近核验**：2026-09-23（v1.3.4 出包并发布；上一批为重启安装退出路径加固 + ReviewStats 类型对账 + 渲染层行类型统一；见第十章变更记录）
...
├── tests/                 # Vitest 单元测试（60 文件 / 1019 用例）
│
...
npm run test             # Vitest（1019 用例；不含覆盖率）
...
| 测试 | 项目已有 Vitest（60 文件 / 1019 用例；纯逻辑 + 组件测试）。新增功能应补 `tests/*.test.ts`，门禁跑 `npm run test` |
