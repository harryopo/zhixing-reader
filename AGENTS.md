...
├── tests/                 # Vitest 单元测试（38 文件 / 850 用例）
│
...
npm run test             # Vitest（850 用例；不含覆盖率）
...
| 测试 | 项目已有 Vitest（38 文件 / 850 用例；纯逻辑 + 组件测试）。新增功能应补 `tests/*.test.ts`，门禁跑 `npm run test` |
